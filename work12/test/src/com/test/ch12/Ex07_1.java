...
public class Ex07_1 {
	...
	public static void main(String[] args) {
		...
		try {
			Vendingmachine3.user();
			
		} catch (MisInput3 e) {
			// TODO Auto-generated catch block
			System.out.println("다시 실행해주세요.");
			e.printStackTrace();
		} catch (NotEnoughBalance3 e) {
			// TODO Auto-generated catch block
			System.out.println("다시 실행해주세요.");
			e.printStackTrace();
		}
...
class Vendingmachine3 {
	...
	public static void user() throws MisInput3, NotEnoughBalance3 {
		...
		while(true) {
				...
				if(run) {
					...
					try {
						if(num>5) {
							throw new MisInput3("메뉴 입력번호 오류");
						}
					}catch(MisInput3 e) {
					...
					if(money >= price[num-1]) {
						...
						try {
							if(money < price[num-1]) {
								throw new NotEnoughBalance3("잔액 부족");
							}
						}catch(NotEnoughBalance3 e){
...
class MisInput3 extends Exception{
	private static final long serialVersionUID = 1L;
	//네트워크 상에서 클래스를 넘겨줄때 시리얼 넘버를 요구함
	//그래서 시리얼 넘버를 작성

	MisInput3(String msg){
		super(msg);
	}
}
 
class NotEnoughBalance3 extends Exception {
	private static final long serialVersionUID = 1L;

	NotEnoughBalance3(String msg) {
		super(msg);
	}
}
